...
class Scorecard:
    ...
    v14_check: dict = field(default_factory=dict)
    v15_check: dict = field(default_factory=dict)
...
# ---------------------------------------------------------------------------
# v13 rubric — robust drivers & validated geometry (Waves AAAAA-HHHHH, D090-D097)
# Each wave traces a v12 (D082-D088) reopening criterion.
# ---------------------------------------------------------------------------


def check_v13_1_1_buckling_constrained() -> tuple[int, str, str]:
    """§1.1 buckling-constrained MMA (λ_crit ≥ λ_safety) (8 pts)."""
    return _mod_and_test(
        "structure_optimizer/core/buckling.py",
...
def check_v13_1_3_extent_adaptive_rho() -> tuple[int, str, str]:
    """§1.3 extent/spread indicator + range-adaptive ρ (8 pts)."""
    return _mod_and_test(
        "structure_optimizer/core/multi_objective_to.py",
...
def check_v13_2_1_d_gumbel() -> tuple[int, str, str]:
    """§2.1 d-dim exchangeable Gumbel copula (8 pts)."""
    return _mod_and_test(
        "structure_optimizer/core/reliability.py",
...
def check_v13_2_2_genz_reorder() -> tuple[int, str, str]:
    """§2.2 Genz variable reordering (8 pts)."""
    return _mod_and_test(
        "structure_optimizer/core/reliability.py",
...
def check_v13_3_1_stacking_sequence() -> tuple[int, str, str]:
    """§3.1 laminate stacking-sequence optimisation (8 pts)."""
    return _mod_and_test(
        "structure_optimizer/core/orthotropic_simp.py",
...
def check_v13_3_2_ruppert_refine() -> tuple[int, str, str]:
    """§3.2 Ruppert quality refinement (Steiner insertion) (7 pts)."""
    return _mod_and_test(
        "structure_optimizer/core/stl_export.py",
...
def check_v13_4_1_test_count_1140() -> tuple[int, str, str]:
    """§4.1 ≥ 1140 tests (4 pts)."""
    n = _pytest_collect_count()
    return (4, "PASS", f"{n} tests collected") if n >= 1140 else (0, "FAIL", f"{n} (need ≥1140)")


def check_v13_4_2_core_coverage_95() -> tuple[int, str, str]:
    """§4.2 core coverage ≥ 95% incl. v13 (4 pts)."""
    pct = _pytest_coverage("structure_optimizer/core")
    return (4, "PASS", f"{pct}% coverage") if pct >= 95.0 else (0, "FAIL", f"{pct}% (need ≥95%)")


def check_v13_4_3_property_tests_56() -> tuple[int, str, str]:
    """§4.3 property tests ≥ 56 (3 pts)."""
    n = _grep_count(r"^def test_property_", "tests/**/*.py")
    return (3, "PASS", f"{n} property tests") if n >= 56 else (0, "FAIL", f"{n} (need ≥56)")


def check_v13_4_4_mutation_75() -> tuple[int, str, str]:
    """§4.4 mutation kill rate ≥ 75% (3 pts)."""
    report = REPO_ROOT / "tests/mutation_report.json"
    if not report.exists():
        return 0, "FAIL", "mutation_report missing"
...
def check_v13_4_5_fingerprints_65() -> tuple[int, str, str]:
    """§4.5 fingerprint DB ≥ 65 (2 pts)."""
    n = len(list((REPO_ROOT / "tests/fingerprints").glob("*.json")))
    return (2, "PASS", f"{n} fingerprints") if n >= 65 else (0, "FAIL", f"{n} (need ≥65)")


def check_v13_4_6_pytest_gate() -> tuple[int, str, str]:
    """§4.6 pytest gate green / D033 mechanism present (2 pts)."""
    has_gate = _grep_count(r"def check_pytest_green", "scripts/test_agent.py") >= 1
    has_adr = _file_exists("docs/decisions/D033-pytest-green-no-regression-gate.md")
    return (
...
def check_v13_4_7_v13_in_agent_ci() -> tuple[int, str, str]:
    """§4.7 v13 rubric referenced in agent + CI (2 pts)."""
    me = (REPO_ROOT / "scripts/test_agent.py").read_text()
    in_agent = "CHECKS_V13" in me
    ci = REPO_ROOT / ".github/workflows/test.yml"
    in_ci = ci.exists() and "v13" in ci.read_text().lower()
    if in_agent and in_ci:
        return 2, "PASS", "v13 in agent + CI"
    if in_agent:
        return 1, "PARTIAL", "v13 in agent, not CI"
    return 0, "FAIL", "v13 not wired"


def check_v13_5_1_buckling_peak_demo() -> tuple[int, str, str]:
    """§5.1 buckling-constrained / peak-binding demo (3 pts)."""
    n = _grep_count(r"buckling_constrained_demo|peak_binding_demo", "**/*.py")
    return (3, "PASS", f"{n} driver-demo refs") if n >= 1 else (0, "FAIL", "no buckling/peak demo")


def check_v13_5_2_extent_demo() -> tuple[int, str, str]:
    """§5.2 extent / range-adaptive demo (3 pts)."""
    n = _grep_count(r"extent_demo|range_adaptive_demo|spread_demo", "**/*.py")
    return (3, "PASS", f"{n} indicator-demo refs") if n >= 1 else (0, "FAIL", "no extent demo")


def check_v13_5_3_gumbel_genz_demo() -> tuple[int, str, str]:
    """§5.3 d-Gumbel / Genz-reorder demo (2 pts)."""
    n = _grep_count(r"gumbel_d_demo|genz_reorder_demo", "**/*.py")
    return (2, "PASS", f"{n} reliability-demo refs") if n >= 1 else (0, "FAIL", "no reliability demo")


def check_v13_5_4_stacking_ruppert_demo() -> tuple[int, str, str]:
    """§5.4 stacking / Ruppert demo (2 pts)."""
    n = _grep_count(r"stacking_demo|ruppert_demo", "**/*.py")
    return (2, "PASS", f"{n} geometry-demo refs") if n >= 1 else (0, "FAIL", "no geometry demo")


def check_v13_6_1_blueprint_v13() -> tuple[int, str, str]:
    """§6.1 blueprint-v13 ≥6 wave ticks (3 pts)."""
    p = REPO_ROOT / "docs/blueprint-v13.md"
    if not p.exists():
        return 0, "FAIL", "blueprint-v13.md missing"
    ticks = p.read_text().count("[x]")
    return (3, "PASS", f"{ticks} ticks") if ticks >= 6 else (0, "PARTIAL", f"{ticks} ticks (need ≥6)")


def check_v13_6_2_tutorial_v13() -> tuple[int, str, str]:
    """§6.2 tutorial v13 ≥4 new sections (3 pts)."""
    p = REPO_ROOT / "docs/tutorial.md"
    if not p.exists():
        return 0, "FAIL", "missing"
    n = len(re.findall(r"^### 23\.\d", p.read_text(), re.MULTILINE))
    return (3, "PASS", f"{n} v13 subsections") if n >= 4 else (0, "PARTIAL", f"{n} (need ≥4)")


def check_v13_6_3_arch_v13() -> tuple[int, str, str]:
    """§6.3 architecture v13 section (3 pts)."""
    p = REPO_ROOT / "docs/architecture.md"
    if not p.exists():
        return 0, "FAIL", "missing"
    txt = p.read_text().lower()
...
CHECKS_V13 = [
    ...
    ("§3", "3.1", "铺层顺序优化", 8, check_v13_3_1_stacking_sequence),
    ("§3", "3.2", "Ruppert 质量细化 (Steiner)", 7, check_v13_3_2_ruppert_refine),
    ("§4", "4.1", "Test count ≥ 1140", 4, check_v13_4_1_test_count_1140),
    ("§4", "4.2", "Core coverage ≥ 95% (含 v13)", 4, check_v13_4_2_core_coverage_95),
    ("§4", "4.3", "Property tests ≥ 56", 3, check_v13_4_3_property_tests_56),
    ("§4", "4.4", "Mutation kill rate ≥ 75%", 3, check_v13_4_4_mutation_75),
    ("§4", "4.5", "Fingerprint DB ≥ 65", 2, check_v13_4_5_fingerprints_65),
    ("§4", "4.6", "pytest gate green (D033)", 2, check_v13_4_6_pytest_gate),
    ("§4", "4.7", "v13 rubric 写入 agent + CI", 2, check_v13_4_7_v13_in_agent_ci),
    ("§5", "5.1", "屈曲约束 / 峰约束 demo", 3, check_v13_5_1_buckling_peak_demo),
    ("§5", "5.2", "extent / range-adaptive demo", 3, check_v13_5_2_extent_demo),
    ("§5", "5.3", "d-Gumbel / Genz-reorder demo", 2, check_v13_5_3_gumbel_genz_demo),
    ("§5", "5.4", "铺层 / Ruppert demo", 2, check_v13_5_4_stacking_ruppert_demo),
    ("§6", "6.1", "blueprint-v13 ≥6 ticks", 3, check_v13_6_1_blueprint_v13),
    ("§6", "6.2", "tutorial v13 ≥4 sections", 3, check_v13_6_2_tutorial_v13),
    ("§6", "6.3", "architecture v13 section", 3, check_v13_6_3_arch_v13),
    ("§6", "6.4", "ADRs D090+ ≥ 7", 3, check_v13_6_4_adrs_v13),
    ("§6", "6.5", "v13 anchors documented", 3, check_v13_6_5_anchors_documented),
]


def run_v13(section_filter: str | None = None) -> Scorecard:
    """Score the v13 rubric in-process."""
    sc = Scorecard(version="v13.0.0")
    sc.items = _score(CHECKS_V13, section_filter)
    return sc
...
# === v15 — embedded constraints & rigorous closure =========================
# Each embedding wave proves the constraint truly binds: (a) feasible under the
# constraint, (b) it CHANGES the unconstrained optimum, (c) opt-in default
# reproduces prior behaviour byte-exact (v14 integration铁律).


def check_v15_1_1_balanced_embed() -> tuple[int, str, str]:
    """§1.1 balanced constraint embedded in optimize_stacking_sequence (8 pts).

    v15-specific token ``balanced_stacking`` (not the pre-existing D101 ``balanced``)
    so the check only passes when D106 actually embeds the constraint."""
    return _mod_and_test(
        "structure_optimizer/core/orthotropic_simp.py",
        r"balanced_stacking",
        r"balanced_stacking",
...
def check_v15_1_2_constrained_selection() -> tuple[int, str, str]:
    """§1.2 constrained discrete angle selection (non-degenerate optimum) (8 pts).

    v15-specific token ``constrained_select`` (not pre-existing D102 ``select_ply_angles``)."""
    return _mod_and_test(
        "structure_optimizer/core/orthotropic_simp.py",
...
def check_v15_1_3_anti_symmetric() -> tuple[int, str, str]:
    """§1.3 anti-symmetric laminate bending-shear decoupling D₁₆=D₂₆=0 (8 pts)."""
    return _mod_and_test(
        "structure_optimizer/core/orthotropic_simp.py",
...
def check_v15_2_1_concentric_export() -> tuple[int, str, str]:
    """§2.1 concentric shells wired into write_stl_cdt_multi_hole(refine) (8 pts).

    v15-specific token ``concentric_export`` (not the pre-existing D103
    ``concentric_shells``) so it only passes when D107 wires it into the STL writer."""
    return _mod_and_test(
        "structure_optimizer/core/stl_export.py",
        r"concentric_export",
        r"concentric_export",
...
def check_v15_2_2_multi_apex_or_qmc() -> tuple[int, str, str]:
    ...
    if mod >= 1:
        return 3, "PARTIAL", "in module, no test"
    return 0, "FAIL", "no multi-apex / QMC-bound"


def check_v15_3_1_kkt_peak_binding() -> tuple[int, str, str]:
    """§3.1 strictly KKT-binding peak-binding (or honest defer w/ probe) (8 pts).

    v15-specific token ``kkt_binding`` (not the pre-existing D104 ``peak_binding``) so it
    only passes when D109 delivers (or documents a provable defer for) strict binding."""
    return _mod_and_test(
        "structure_optimizer/core/freq_response.py",
...
def check_v15_3_2_copula_general_rosenblatt() -> tuple[int, str, str]:
    """§3.2 copula system reliability into general Rosenblatt / multi-family (8 pts).

    v15-specific token ``copula_general`` (not the pre-existing D098
    ``system_reliability_series_copula`` / D069 ``copula_rosenblatt``)."""
    return _mod_and_test(
        "structure_optimizer/core/reliability.py",
...
def check_v15_4_1_test_count_1245() -> tuple[int, str, str]:
    """§4.1 ≥ 1245 tests (4 pts)."""
    n = _pytest_collect_count()
    return (4, "PASS", f"{n} tests collected") if n >= 1245 else (0, "FAIL", f"{n} (need ≥1245)")


def check_v15_4_2_core_coverage_95() -> tuple[int, str, str]:
    """§4.2 core coverage ≥ 95% incl. v15 (4 pts)."""
    pct = _pytest_coverage("structure_optimizer/core")
    return (4, "PASS", f"{pct}% coverage") if pct >= 95.0 else (0, "FAIL", f"{pct}% (need ≥95%)")


def check_v15_4_3_property_tests_62() -> tuple[int, str, str]:
    """§4.3 property tests ≥ 62 (3 pts)."""
    n = _grep_count(r"^def test_property_", "tests/**/*.py")
    return (3, "PASS", f"{n} property tests") if n >= 62 else (0, "FAIL", f"{n} (need ≥62)")


def check_v15_4_4_mutation_75() -> tuple[int, str, str]:
    """§4.4 mutation kill rate ≥ 75% (3 pts)."""
    report = REPO_ROOT / "tests/mutation_report.json"
    if not report.exists():
        return 0, "FAIL", "mutation_report missing"
...
def check_v15_4_5_fingerprints_75() -> tuple[int, str, str]:
    """§4.5 fingerprint DB ≥ 75 (2 pts)."""
    n = len(list((REPO_ROOT / "tests/fingerprints").glob("*.json")))
    return (2, "PASS", f"{n} fingerprints") if n >= 75 else (0, "FAIL", f"{n} (need ≥75)")


def check_v15_4_6_pytest_gate() -> tuple[int, str, str]:
    """§4.6 pytest gate green / D033 mechanism present (2 pts)."""
    has_gate = _grep_count(r"def check_pytest_green", "scripts/test_agent.py") >= 1
    has_adr = _file_exists("docs/decisions/D033-pytest-green-no-regression-gate.md")
    return (
...
def check_v15_4_7_v15_in_agent_ci() -> tuple[int, str, str]:
    """§4.7 v15 rubric referenced in agent + CI (2 pts)."""
    me = (REPO_ROOT / "scripts/test_agent.py").read_text()
    in_agent = "CHECKS_V15" in me
    ci = REPO_ROOT / ".github/workflows/test.yml"
    in_ci = ci.exists() and "v15" in ci.read_text().lower()
    if in_agent and in_ci:
        return 2, "PASS", "v15 in agent + CI"
    if in_agent:
        return 1, "PARTIAL", "v15 in agent, not CI"
    return 0, "FAIL", "v15 not wired"


def check_v15_5_1_laminate_demo() -> tuple[int, str, str]:
    """§5.1 balanced-stacking / constrained-select demo (3 pts)."""
    n = _grep_count(r"balanced_stacking_demo|constrained_select_demo", "**/*.py")
    return (3, "PASS", f"{n} laminate-demo refs") if n >= 1 else (0, "FAIL", "no laminate demo")


def check_v15_5_2_geometry_demo() -> tuple[int, str, str]:
    """§5.2 concentric-export / multi-apex demo (3 pts)."""
    n = _grep_count(r"concentric_export_demo|multi_apex_demo", "**/*.py")
    return (3, "PASS", f"{n} geometry-demo refs") if n >= 1 else (0, "FAIL", "no geometry demo")


def check_v15_5_3_anti_copula_demo() -> tuple[int, str, str]:
    """§5.3 anti-symmetric / copula-rosenblatt demo (2 pts)."""
    n = _grep_count(r"anti_symmetric_demo|copula_rosenblatt_demo", "**/*.py")
    return (2, "PASS", f"{n} anti/copula-demo refs") if n >= 1 else (0, "FAIL", "no anti/copula demo")


def check_v15_5_4_peak_active_demo() -> tuple[int, str, str]:
    """§5.4 peak-binding-active demo (2 pts)."""
    n = _grep_count(r"peak_binding_active_demo|kkt_peak_demo", "**/*.py")
    return (2, "PASS", f"{n} peak-demo refs") if n >= 1 else (0, "FAIL", "no peak demo")


def check_v15_6_1_blueprint_v15() -> tuple[int, str, str]:
    """§6.1 blueprint-v15 ≥6 wave ticks (3 pts)."""
    p = REPO_ROOT / "docs/blueprint-v15.md"
    if not p.exists():
        return 0, "FAIL", "blueprint-v15.md missing"
    ticks = p.read_text().count("[x]")
    return (3, "PASS", f"{ticks} ticks") if ticks >= 6 else (0, "PARTIAL", f"{ticks} ticks (need ≥6)")


def check_v15_6_2_tutorial_v15() -> tuple[int, str, str]:
    """§6.2 tutorial v15 ≥4 new sections (3 pts)."""
    p = REPO_ROOT / "docs/tutorial.md"
    if not p.exists():
        return 0, "FAIL", "missing"
    n = len(re.findall(r"^### 25\.\d", p.read_text(), re.MULTILINE))
    return (3, "PASS", f"{n} v15 subsections") if n >= 4 else (0, "PARTIAL", f"{n} (need ≥4)")


def check_v15_6_3_arch_v15() -> tuple[int, str, str]:
    """§6.3 architecture v15 section (3 pts)."""
    p = REPO_ROOT / "docs/architecture.md"
    if not p.exists():
        return 0, "FAIL", "missing"
    txt = p.read_text().lower()
...
def check_v15_6_5_anchors_documented() -> tuple[int, str, str]:
    """§6.5 v15 quantitative anchors present in tests (3 pts).

    v15-specific tokens only (the wave-introduced identifiers), so this reflects real
    v15 test anchors, not pre-existing v14/earlier ones."""
    n = _grep_count(
        r"balanced_stacking|constrained_select|anti_symmetric|concentric_export|multi_apex|kkt_binding|copula_general",
        "tests/**/*.py",
    )
    return (3, "PASS", f"{n} v15-anchor refs") if n >= 3 else (0, "PARTIAL", f"{n} (need ≥3)")


CHECKS_V15 = [
    ("§1", "1.1", "balanced 约束嵌入 optimize_stacking_sequence", 8, check_v15_1_1_balanced_embed),
    ("§1", "1.2", "约束化离散角选择（非退化）", 8, check_v15_1_2_constrained_selection),
    ("§1", "1.3", "anti-symmetric 弯-剪解耦 D₁₆=D₂₆=0", 8, check_v15_1_3_anti_symmetric),
    ("§2", "2.1", "concentric shells 接入 write_stl_cdt_multi_hole", 8, check_v15_2_1_concentric_export),
    ("§2", "2.2", "多-apex / 确定性 QMC 界（或诚实 defer）", 7, check_v15_2_2_multi_apex_or_qmc),
    ("§3", "3.1", "严格 KKT-binding peak-binding（或诚实 defer）", 8, check_v15_3_1_kkt_peak_binding),
    ("§3", "3.2", "copula 系统可靠性 general Rosenblatt / 多 family", 8, check_v15_3_2_copula_general_rosenblatt),
    ("§4", "4.1", "Test count ≥ 1245", 4, check_v15_4_1_test_count_1245),
    ("§4", "4.2", "Core coverage ≥ 95% (含 v15)", 4, check_v15_4_2_core_coverage_95),
    ("§4", "4.3", "Property tests ≥ 62", 3, check_v15_4_3_property_tests_62),
    ("§4", "4.4", "Mutation kill rate ≥ 75%", 3, check_v15_4_4_mutation_75),
    ("§4", "4.5", "Fingerprint DB ≥ 75", 2, check_v15_4_5_fingerprints_75),
    ("§4", "4.6", "pytest gate green (D033)", 2, check_v15_4_6_pytest_gate),
    ("§4", "4.7", "v15 rubric 写入 agent + CI", 2, check_v15_4_7_v15_in_agent_ci),
    ("§5", "5.1", "balanced-stacking / constrained-select demo", 3, check_v15_5_1_laminate_demo),
    ("§5", "5.2", "concentric-export / multi-apex demo", 3, check_v15_5_2_geometry_demo),
    ("§5", "5.3", "anti-symmetric / copula-rosenblatt demo", 2, check_v15_5_3_anti_copula_demo),
    ("§5", "5.4", "peak-binding-active demo", 2, check_v15_5_4_peak_active_demo),
    ("§6", "6.1", "blueprint-v15 ≥6 ticks", 3, check_v15_6_1_blueprint_v15),
    ("§6", "6.2", "tutorial v15 ≥4 sections", 3, check_v15_6_2_tutorial_v15),
    ("§6", "6.3", "architecture v15 section", 3, check_v15_6_3_arch_v15),
    ("§6", "6.4", "ADRs D106+ ≥ 7", 3, check_v15_6_4_adrs_v15),
    ("§6", "6.5", "v15 anchors documented", 3, check_v15_6_5_anchors_documented),
]


def run_v15(section_filter: str | None = None) -> Scorecard:
    """Score the v15 rubric in-process."""
    sc = Scorecard(version="v15.0.0")
    sc.items = _score(CHECKS_V15, section_filter)
    return sc


# --- v16 rubric: deep embedding & exact generalization -----------------------
# Each capability check greps a v16-specific NEW symbol (not a pre-existing v15
# identifier), so all §1-§3 checks correctly FAIL 0/8 before implementation.


def check_v16_1_1_antisym_embed() -> tuple[int, str, str]:
    """§1.1 anti-symmetric ordering constraint embedded in optimize_stacking_sequence (8 pts).

    v16-specific token ``bending_shear_decoupled`` (not the pre-existing D110
    ``make_antisymmetric_laminate``) so it only passes when D114 embeds the constraint."""
    return _mod_and_test(
        "structure_optimizer/core/orthotropic_simp.py",
...
def check_v16_1_2_cbc_genz() -> tuple[int, str, str]:
    """§1.2 CBC deterministic vector wired into Genz MVN CDF (8 pts).

    v16-specific token ``genz_mvn_cdf_cbc`` (not the pre-existing D112 ``cbc_korobov`` /
    D086 ``genz_mvn_cdf_lattice``)."""
    return _mod_and_test(
        "structure_optimizer/core/reliability.py",
...
def check_v16_1_3_copula_marginals() -> tuple[int, str, str]:
    """§1.3 mixture-copula system reliability with general non-normal marginals (8 pts).

    v16-specific token ``copula_marginals`` (not the pre-existing D111
    ``multi_family_copula`` / D098 ``system_reliability_series_copula``)."""
    return _mod_and_test(
        "structure_optimizer/core/reliability.py",
...
def check_v16_2_1_alpha_korobov() -> tuple[int, str, str]:
    """§2.1 α≥2 higher-smoothness weighted-Korobov worst-case error (8 pts).

    v16-specific token ``_korobov_kernel_omega_alpha`` (not the pre-existing D112
...
def check_v16_2_2_fast_cbc() -> tuple[int, str, str]:
    """§2.2 fast-CBC (FFT) or honest defer (7 pts)."""
    mod = _grep_count(r"fast_cbc|cbc_fft", "structure_optimizer/core/*.py")
    test = _grep_count(r"fast_cbc|cbc_fft|fast.*cbc", "tests/**/*.py")
    if mod >= 1 and test >= 1:
        return 7, "PASS", "fast-CBC FFT + test"
    if mod >= 1:
        return 3, "PARTIAL", "in module, no test"
    return 0, "FAIL", "no fast-CBC"


def check_v16_3_1_exact_multiplier() -> tuple[int, str, str]:
    """§3.1 exact KKT multiplier for peak-binding (or honest defer) (8 pts).

    v16-specific token ``exact_multiplier`` (not the pre-existing D109 ``active_multiplier``
    proxy)."""
    return _mod_and_test(
        "structure_optimizer/core/freq_response.py",
...
def check_v16_3_2_parallel_copula() -> tuple[int, str, str]:
    """§3.2 copula parallel / general system reliability (8 pts).

    v16-specific token ``parallel_copula`` (not the pre-existing D098 series-only path)."""
    return _mod_and_test(
        "structure_optimizer/core/reliability.py",
...
def check_v16_4_1_test_count_1290() -> tuple[int, str, str]:
    """§4.1 ≥ 1290 tests (4 pts)."""
    n = _pytest_collect_count()
    return (4, "PASS", f"{n} tests collected") if n >= 1290 else (0, "FAIL", f"{n} (need ≥1290)")


def check_v16_4_2_core_coverage_95() -> tuple[int, str, str]:
    """§4.2 core coverage ≥ 95% incl. v16 (4 pts)."""
    pct = _pytest_coverage("structure_optimizer/core")
    return (4, "PASS", f"{pct}% coverage") if pct >= 95.0 else (0, "FAIL", f"{pct}% (need ≥95%)")


def check_v16_4_3_property_tests_65() -> tuple[int, str, str]:
    """§4.3 property tests ≥ 65 (3 pts)."""
    n = _grep_count(r"^def test_property_", "tests/**/*.py")
    return (3, "PASS", f"{n} property tests") if n >= 65 else (0, "FAIL", f"{n} (need ≥65)")


def check_v16_4_4_mutation_75() -> tuple[int, str, str]:
    """§4.4 mutation kill rate ≥ 75% (3 pts)."""
    report = REPO_ROOT / "tests/mutation_report.json"
    if not report.exists():
        return 0, "FAIL", "mutation_report missing"
...
def check_v16_4_5_fingerprints_80() -> tuple[int, str, str]:
    """§4.5 fingerprint DB ≥ 80 (2 pts)."""
    n = len(list((REPO_ROOT / "tests/fingerprints").glob("*.json")))
    return (2, "PASS", f"{n} fingerprints") if n >= 80 else (0, "FAIL", f"{n} (need ≥80)")


def check_v16_4_6_pytest_gate() -> tuple[int, str, str]:
    """§4.6 pytest gate green / D033 mechanism present (2 pts)."""
    has_gate = _grep_count(r"def check_pytest_green", "scripts/test_agent.py") >= 1
    has_adr = _file_exists("docs/decisions/D033-pytest-green-no-regression-gate.md")
    return (
...
def check_v16_4_7_v16_in_agent_ci() -> tuple[int, str, str]:
    """§4.7 v16 rubric referenced in agent + CI (2 pts)."""
    me = (REPO_ROOT / "scripts/test_agent.py").read_text()
    in_agent = "CHECKS_V16" in me
    ci = REPO_ROOT / ".github/workflows/test.yml"
    in_ci = ci.exists() and "v16" in ci.read_text().lower()
    if in_agent and in_ci:
        return 2, "PASS", "v16 in agent + CI"
    if in_agent:
        return 1, "PARTIAL", "v16 in agent, not CI"
    return 0, "FAIL", "v16 not wired"


def check_v16_5_1_embed_demo() -> tuple[int, str, str]:
    """§5.1 anti-symmetry-embed / cbc-lattice demo (3 pts)."""
    n = _grep_count(r"antisym_embed_demo|cbc_lattice_demo", "**/*.py")
    return (3, "PASS", f"{n} embed-demo refs") if n >= 1 else (0, "FAIL", "no embed demo")


def check_v16_5_2_general_demo() -> tuple[int, str, str]:
    """§5.2 general-marginals / parallel-system demo (3 pts)."""
    n = _grep_count(r"general_marginal_demo|parallel_system_demo", "**/*.py")
    return (3, "PASS", f"{n} general-demo refs") if n >= 1 else (0, "FAIL", "no general demo")


def check_v16_5_3_qmc_demo() -> tuple[int, str, str]:
    """§5.3 alpha2-korobov / fast-cbc demo (2 pts)."""
    n = _grep_count(r"alpha_korobov_demo|fast_cbc_demo", "**/*.py")
    return (2, "PASS", f"{n} qmc-demo refs") if n >= 1 else (0, "FAIL", "no qmc demo")


def check_v16_5_4_kkt_demo() -> tuple[int, str, str]:
    """§5.4 exact-kkt-multiplier demo (2 pts)."""
    n = _grep_count(r"exact_kkt_demo|exact_multiplier_demo", "**/*.py")
    return (2, "PASS", f"{n} kkt-demo refs") if n >= 1 else (0, "FAIL", "no kkt demo")


def check_v16_6_1_blueprint_v16() -> tuple[int, str, str]:
    """§6.1 blueprint-v16 ≥6 wave ticks (3 pts)."""
    p = REPO_ROOT / "docs/blueprint-v16.md"
    if not p.exists():
        return 0, "FAIL", "blueprint-v16.md missing"
    ticks = p.read_text().count("[x]")
    return (3, "PASS", f"{ticks} ticks") if ticks >= 6 else (0, "PARTIAL", f"{ticks} ticks (need ≥6)")


def check_v16_6_2_tutorial_v16() -> tuple[int, str, str]:
    """§6.2 tutorial v16 ≥4 new sections (3 pts)."""
    p = REPO_ROOT / "docs/tutorial.md"
    if not p.exists():
        return 0, "FAIL", "missing"
    n = len(re.findall(r"^### 26\.\d", p.read_text(), re.MULTILINE))
    return (3, "PASS", f"{n} v16 subsections") if n >= 4 else (0, "PARTIAL", f"{n} (need ≥4)")


def check_v16_6_3_arch_v16() -> tuple[int, str, str]:
    """§6.3 architecture v16 section (3 pts)."""
    p = REPO_ROOT / "docs/architecture.md"
    if not p.exists():
        return 0, "FAIL", "missing"
    txt = p.read_text().lower()
...
def check_v16_6_5_anchors_documented() -> tuple[int, str, str]:
    """§6.5 v16 quantitative anchors present in tests (3 pts).

    v16-specific tokens only (the wave-introduced identifiers)."""
    n = _grep_count(
        r"bending_shear_decoupled|genz_mvn_cdf_cbc|copula_marginals|_korobov_kernel_omega_alpha|fast_cbc|exact_multiplier|parallel_copula",
        "tests/**/*.py",
    )
    return (3, "PASS", f"{n} v16-anchor refs") if n >= 3 else (0, "PARTIAL", f"{n} (need ≥3)")


CHECKS_V16 = [
    ("§1", "1.1", "anti-symmetric 排序约束嵌入 optimize_stacking_sequence", 8, check_v16_1_1_antisym_embed),
    ("§1", "1.2", "CBC 确定性向量接入 genz_mvn_cdf", 8, check_v16_1_2_cbc_genz),
    ("§1", "1.3", "mixture-copula 一般非正态边缘", 8, check_v16_1_3_copula_marginals),
    ("§2", "2.1", "α≥2 高阶光滑 Korobov 最坏情况误差", 8, check_v16_2_1_alpha_korobov),
    ("§2", "2.2", "fast-CBC FFT（或诚实 defer）", 7, check_v16_2_2_fast_cbc),
    ("§3", "3.1", "peak-binding 精确 KKT 乘子（或诚实 defer）", 8, check_v16_3_1_exact_multiplier),
    ("§3", "3.2", "copula 并联/一般系统可靠性", 8, check_v16_3_2_parallel_copula),
    ("§4", "4.1", "Test count ≥ 1290", 4, check_v16_4_1_test_count_1290),
    ("§4", "4.2", "Core coverage ≥ 95% (含 v16)", 4, check_v16_4_2_core_coverage_95),
    ("§4", "4.3", "Property tests ≥ 65", 3, check_v16_4_3_property_tests_65),
    ("§4", "4.4", "Mutation kill rate ≥ 75%", 3, check_v16_4_4_mutation_75),
    ("§4", "4.5", "Fingerprint DB ≥ 80", 2, check_v16_4_5_fingerprints_80),
    ("§4", "4.6", "pytest gate green (D033)", 2, check_v16_4_6_pytest_gate),
    ("§4", "4.7", "v16 rubric 写入 agent + CI", 2, check_v16_4_7_v16_in_agent_ci),
    ("§5", "5.1", "anti-symmetry-embed / cbc-lattice demo", 3, check_v16_5_1_embed_demo),
    ("§5", "5.2", "general-marginals / parallel-system demo", 3, check_v16_5_2_general_demo),
    ("§5", "5.3", "alpha2-korobov / fast-cbc demo", 2, check_v16_5_3_qmc_demo),
    ("§5", "5.4", "exact-kkt-multiplier demo", 2, check_v16_5_4_kkt_demo),
    ("§6", "6.1", "blueprint-v16 ≥6 ticks", 3, check_v16_6_1_blueprint_v16),
    ("§6", "6.2", "tutorial v16 ≥4 sections", 3, check_v16_6_2_tutorial_v16),
    ("§6", "6.3", "architecture v16 section", 3, check_v16_6_3_arch_v16),
    ("§6", "6.4", "ADRs D114+ ≥ 7", 3, check_v16_6_4_adrs_v16),
    ("§6", "6.5", "v16 anchors documented", 3, check_v16_6_5_anchors_documented),
]


def run_v16(section_filter: str | None = None) -> Scorecard:
    """Score the v16 rubric in-process."""
    sc = Scorecard(version="v16.0.0")
    sc.items = _score(CHECKS_V16, section_filter)
    return sc
...
def check_v12_no_regression() -> dict:
    """v12 must remain 100/100 for a v13 release (true in-process re-score)."""
    sc_v12 = run_v12()
    return {
...
def check_v14_no_regression() -> dict:
    """v14 must remain 100/100 for a v15 release (true in-process re-score)."""
    sc_v14 = run_v14()
    return {
        "score": sc_v14.total_earned,
        "max": sc_v14.total_max,
        "regression": sc_v14.total_earned < 100,
    }


def check_v15_no_regression() -> dict:
    """v15 must remain 100/100 for a v16 release (true in-process re-score)."""
    sc_v15 = run_v15()
    return {
        "score": sc_v15.total_earned,
        "max": sc_v15.total_max,
        "regression": sc_v15.total_earned < 100,
    }
...
def run_all(section_filter: str | None = None, rubric: str = "v5", run_pytest_gate: bool = True) -> Scorecard:
    ...
    if rubric == "v4":
        ...
        sc = run_v15(section_filter)
    else:  # v16
        sc = run_v16(section_filter)
    sc.v1_check = check_v1_no_regression()
    sc.v2_check = check_v2_no_regression()
    sc.v3_check = check_v3_no_regression()
    if rubric in ("v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12", "v13", "v14", "v15", "v16"):
        sc.v4_check = check_v4_no_regression()
    if rubric in ("v6", "v7", "v8", "v9", "v10", "v11", "v12", "v13", "v14", "v15", "v16"):
        sc.v5_check = check_v5_no_regression()
    if rubric in ("v7", "v8", "v9", "v10", "v11", "v12", "v13", "v14", "v15", "v16"):
        sc.v6_check = check_v6_no_regression()
    if rubric in ("v8", "v9", "v10", "v11", "v12", "v13", "v14", "v15", "v16"):
        sc.v7_check = check_v7_no_regression()
    if rubric in ("v9", "v10", "v11", "v12", "v13", "v14", "v15", "v16"):
        sc.v8_check = check_v8_no_regression()
    if rubric in ("v10", "v11", "v12", "v13", "v14", "v15", "v16"):
        sc.v9_check = check_v9_no_regression()
    if rubric in ("v11", "v12", "v13", "v14", "v15", "v16"):
        sc.v10_check = check_v10_no_regression()
    if rubric in ("v12", "v13", "v14", "v15", "v16"):
        sc.v11_check = check_v11_no_regression()
    if rubric in ("v13", "v14", "v15", "v16"):
        sc.v12_check = check_v12_no_regression()
    if rubric in ("v14", "v15", "v16"):
        sc.v13_check = check_v13_no_regression()
    if rubric in ("v15", "v16"):
        sc.v14_check = check_v14_no_regression()
    if rubric == "v16":
        sc.v15_check = check_v15_no_regression()
    if run_pytest_gate:
...
def print_summary(sc: Scorecard) -> None:
    ...
    regressions = [
        ...
        sc.v14_check.get("regression") if sc.v14_check else False,
...
def main() -> int:
    ...
    parser.add_argument(
        "--rubric",
        choices=["v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12", "v13", "v14", "v15", "v16"],
        default="v5",
        help="which rubric to score (default v5; v4-v14 scorable for regression checks)",
    )
    parser.add_argument(
        "--output",
        default=None,
    ...
    if sc.v14_check:
        payload["v14_check"] = sc.v14_check
    if sc.v15_check:
        payload["v15_check"] = sc.v15_check
    if sc.pytest_check:
    ...
    if args.strict:
        regression = any(
            [
                ...
                sc.v13_check.get("regression") if sc.v13_check else False,
                sc.v14_check.get("regression") if sc.v14_check else False,
                sc.v15_check.get("regression") if sc.v15_check else False,
                sc.pytest_check.get("regression") if sc.pytest_check else False,
            ]
        )
