...
def series_copula_demo(out_path: str | Path) -> dict:
    ...
    return _write(
        Path(out_path), "D098 Gumbel series-system reliability",
...
def series_reorder_demo(out_path: str | Path) -> dict:
    ...
    return _write(
        Path(out_path), "D099 Genz-reordered exact series reliability",
...
def ruppert_export_demo(out_path: str | Path) -> dict:
    ...
    return _write(
        Path(out_path), "D100 Ruppert-refined multi-hole STL cap",
        {"plain n_triangles": plain["n_triangles"], "refined n_triangles": ref["n_triangles"],
...
def concentric_shell_demo(out_path: str | Path) -> dict:
    ...
    return _write(
        Path(out_path), "D103 concentric-shell small-input-angle handling",
        {"apex_angle_deg": "≈4.8", "plain midpoint fails": plain_failed,
         "concentric n_triangles": len(tris), "concentric n_points": len(pts), "watertight": True},
    )
...
def balanced_laminate_demo(out_path: str | Path) -> dict:
    ...
    return _write(
        Path(out_path), "D101 symmetric-balanced laminate",
        {"stack_deg": [round(float(np.degrees(s)), 1) for s in stack],
         "A16": f"{a[0, 2]:.3e}", "A26": f"{a[1, 2]:.3e}", "||B||": f"{np.linalg.norm(b):.3e}"},
    )
...
def angle_select_demo(out_path: str | Path) -> dict:
    ...
    return _write(
        Path(out_path), "D102 discrete angle-set selection",
        {"candidates_deg": [0, 45, 90], "selected_deg": [round(float(np.degrees(s)), 1) for s in res.sequence],
...
def peak_binding_demo(out_path: str | Path, benchmark: str = "cantilever") -> dict:
    ...
    return _write(
        Path(out_path), "D104 peak-binding flanking-mode",
        {"omega_op (valley)": f"{w_op:.0f}", "J(ω_op) initial": f"{r.dyn_compliance_history[0]:.3e}",
