...
- [x] GGGGGGGG — fast-CBC（Nuyens–Cools FFT）点阵构造（fast_cbc_korobov_generating_vector：素数 N 按原根重排 (Z/N)^× ⟹ 每分量 CBC 目标=循环相关 ⟹ 一对 rfft/irfft O(N log N) 全候选，总 O(d·N·log N)；canonical 对称 min-g tie-break ⟹ 确定性无 RNG；headline FFT 相关==直接 O(N²) 相关 1e-10 + e≤教科书 Korobov（6 素数）+ e≤min 30 随机 + 确定性逐位 + [--run-slow] N=1021 ≥10× 加速；**probe-then-decide**：literal blueprint anchor "fast z==naive z" **不可达**——B₂(1−t)=B₂(t) ⟹ g↔N−g 精确对称 ⟹ 最优 2^{d−1} 成员集、naive 靠浮点求和顺序选/fast 靠 canonical 选、等优；强行匹配 20/21、第 21 例需伪精度 ⟹ 改锚可达真理；仅素数 N、本仓库加速学术性；纯新增不动 naive；D120）
- [x] HHHHHHHH — v16 收口（scripts/v16_demos.py 7 真实 demo 覆盖 §5 四 token；fingerprints 75→80 +5 bit-exact recipe+present-set；property 63→65 +tests/test_property_v16.py；architecture §26；CI v16 --strict step；tests/v16_scorecard.json；test count 1323≥1290 / property 65 / fingerprint 80 双 tier / mutation 80%；新代码 ~150 stmt ~98% 覆盖（仅 2 防御 unreachable）；authoritative --rubric v16 --strict 目标 100/100 + v4-v15 regression=False + D033 gate；D121）
